class Admin::InvoicesController < Admin::BaseController
  ...
  before_action :set_invoice, only: [ :show, :update, :mark_check_issued, :mark_check_cashed ]
  ...
  def index
    ...
    respond_to do |format|
      format.html
      format.csv do
        send_data generate_csv(@invoices),
          filename: "invoices-#{Time.current.strftime("%Y%m%d")}.csv",
          type: "text/csv"
      end
    end
  end
