class Admin::PaperApplicationsController < Admin::BaseController
  ...
  def log_file_and_form_params
    ...
    Rails.logger.debug 'income_proof present: ' + params[:income_proof].present?.to_s
    Rails.logger.debug 'residency_proof present: ' + params[:residency_proof].present?.to_s
    if params[:income_proof].present?
      Rails.logger.debug 'income_proof class: ' + params[:income_proof].class.to_s
  ...
  def generate_success_message(application)
    if application.proof_reviews.where(status: :rejected).any?
      ...
      if rejected_proofs.any?
        message = 'Paper application successfully submitted with ' + rejected_proofs.length.to_s + ' rejected '
  ...
  def cast_boolean_params
    ...
    cast_boolean_for(params[:constituent], %i[hearing_disability vision_disability speech_disability mobility_disability cognition_disability is_guardian])
    cast_boolean_for(params[:application], %i[self_certify_disability maryland_resident terms_accepted information_verified medical_release_authorized])
  end
