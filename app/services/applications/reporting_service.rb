module Applications
  class ReportingService < BaseService
    ...
    def generate_dashboard_data
      ...
      # Applications data
      data[:current_fy_applications] = Application.where(created_at: data[:current_fy_start]..data[:current_fy_end]).count
      data[:previous_fy_applications] = Application.where(created_at: data[:previous_fy_start]..data[:previous_fy_end]).count

      # Draft applications (started but not submitted)
      data[:current_fy_draft_applications] = Application.where(status: :draft, created_at: data[:current_fy_start]..data[:current_fy_end]).count
      data[:previous_fy_draft_applications] = Application.where(status: :draft, created_at: data[:previous_fy_start]..data[:previous_fy_end]).count
      ...
      # Unredeemed vouchers
      data[:current_fy_unredeemed_vouchers] = Voucher.where(created_at: data[:current_fy_start]..data[:current_fy_end], status: :active).count
      data[:previous_fy_unredeemed_vouchers] = Voucher.where(created_at: data[:previous_fy_start]..data[:previous_fy_end], status: :active).count

      # Voucher values
      data[:current_fy_voucher_value] = Voucher.where(created_at: data[:current_fy_start]..data[:current_fy_end]).sum(:initial_value)
      data[:previous_fy_voucher_value] = Voucher.where(created_at: data[:previous_fy_start]..data[:previous_fy_end]).sum(:initial_value)

      # Training sessions
      data[:current_fy_trainings] = TrainingSession.where(created_at: data[:current_fy_start]..data[:current_fy_end]).count
      data[:previous_fy_trainings] = TrainingSession.where(created_at: data[:previous_fy_start]..data[:previous_fy_end]).count

      # Evaluation sessions
      data[:current_fy_evaluations] = Evaluation.where(created_at: data[:current_fy_start]..data[:current_fy_end]).count
      data[:previous_fy_evaluations] = Evaluation.where(created_at: data[:previous_fy_start]..data[:previous_fy_end]).count
      ...
      data[:mfr_applications_approved] = Application.where(created_at: data[:previous_fy_start]..data[:previous_fy_end], status: :approved).count
      ...
      data[:applications_chart_data] = {
        current: { "Applications" => data[:current_fy_applications], "Draft Applications" => data[:current_fy_draft_applications] },
        previous: { "Applications" => data[:previous_fy_applications], "Draft Applications" => data[:previous_fy_draft_applications] }
      }

      # Chart data for vouchers
      data[:vouchers_chart_data] = {
        current: { "Vouchers Issued" => data[:current_fy_vouchers], "Unredeemed Vouchers" => data[:current_fy_unredeemed_vouchers] },
        previous: { "Vouchers Issued" => data[:previous_fy_vouchers], "Unredeemed Vouchers" => data[:previous_fy_unredeemed_vouchers] }
      }

      # Chart data for services
      data[:services_chart_data] = {
        current: { "Training Sessions" => data[:current_fy_trainings], "Evaluation Sessions" => data[:current_fy_evaluations] },
        previous: { "Training Sessions" => data[:previous_fy_trainings], "Evaluation Sessions" => data[:previous_fy_evaluations] }
      }

      # Chart data for MFR
      data[:mfr_chart_data] = {
        current: { "Applications Approved" => data[:mfr_applications_approved], "Vouchers Issued" => data[:mfr_vouchers_issued] },
    ...
    def generate_index_data
      ...
      data[:proofs_needing_review_count] = (income_proofs_pending.pluck(:id) + residency_proofs_pending.pluck(:id)).uniq.count
      ...
      data[:in_review_count] = Application.where(status: [ "submitted", "in_review" ]).count
      ...
      data[:draft_count] = Application.where(status: "draft").count
      data[:in_progress_count] = Application.where(status: [ "submitted", "in_review" ]).count
      data[:approved_count] = Application.where(status: "approved").count
