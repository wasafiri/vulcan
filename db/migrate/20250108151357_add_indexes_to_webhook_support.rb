...
class AddIndexesToWebhookSupport < ActiveRecord::Migration[8.0]
  def change
    ...
    add_index :applications, [ :medical_provider_email, :status ], name: "index_applications_on_medical_provider_email_and_status"
    ...
    if table_exists?(:proof_submission_audits)
      add_index :proof_submission_audits, [ :application_id, :created_at ], name: "index_proof_submission_audits_on_application_id_and_created_at"
    end
  end
end
