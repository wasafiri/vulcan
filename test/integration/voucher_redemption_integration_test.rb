...
class VoucherRedemptionIntegrationTest < ActionDispatch::IntegrationTest
  ...
  test "full voucher redemption flow with database integrity verification" do
    ...
    assert_equal transaction_products_before + 2, VoucherTransactionProduct.count, "New transaction products should be created"
    ...
    @application.reload
    assert_equal application_products_before + 2, @application.products.count, "Application should have new products associated"
  ...
  test "voucher redemption handles validation errors appropriately" do
    ...
    follow_redirect!
    assert_match /exceeds remaining voucher balance|Amount exceeds remaining voucher balance|Invalid voucher code/, flash[:alert]
