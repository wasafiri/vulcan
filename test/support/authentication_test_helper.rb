...
module AuthenticationTestHelper
  ...
  def authenticate_user!(user = nil)
    ...
    if defined?(response) && response.present? && response.redirect? && response.location.include?('sign_in')
      raise "Expected to be authenticated as #{user.email}, but was redirected to sign in"
    end
    ...
    if defined?(@controller) && @controller.respond_to?(:current_user, true)
      ...
      if actual_user.nil?
        raise "Expected to be authenticated as #{user.email}, but current_user is nil"
      elsif actual_user.id != user.id
        raise "Expected to be authenticated as #{user.email}, but was authenticated as #{actual_user.email}"
      end
    end
