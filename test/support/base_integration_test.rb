...
class BaseIntegrationTest < ActionDispatch::IntegrationTest
  ...
  def assert_flash_message(type, message)
    assert_equal message, flash[type.to_sym],
                 "Expected flash #{type} to be '#{message}', but was '#{flash[type.to_sym]}'"
  end
  ...
  def assert_record_created(model_class, &block)
    assert_difference("#{model_class}.count", 1, &block)
  end

  # Helper method to verify that a record was not created
  def assert_record_not_created(model_class, &block)
    assert_no_difference("#{model_class}.count", &block)
  end
  ...
  def assert_record_not_updated(record, attributes)
    attributes.transform_values { |attr| record.send(attr) }
    yield
    record.reload
  ...
  def assert_record_deleted(model_class, &block)
    assert_difference("#{model_class}.count", -1, &block)
  end

  # Helper method to verify that a record was not deleted
  def assert_record_not_deleted(model_class, &block)
    assert_no_difference("#{model_class}.count", &block)
  end
end
