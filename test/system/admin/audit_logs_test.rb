...
class Admin::AuditLogsTest < ApplicationSystemTestCase
  setup do
  ...
  test "audit logs correctly show proof review actions without duplicates" do
    ...
    if page.has_css?("#audit-logs")
      within "#audit-logs" do
        ...
        income_approved_count = all("tr").count { |tr| tr.text.include?("Income proof") && tr.text.include?("approved") }
    ...
    if page.has_css?("#audit-logs")
      within "#audit-logs" do
        ...
        residency_rejected_count = all("tr").count { |tr| tr.text.include?("Residency proof") && tr.text.include?("rejected") }
